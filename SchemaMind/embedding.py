...
modelname = r"modelcache/codebertbase"

try:
    tokenizer = RobertaTokenizer.from_pretrained(modelname, local_files_only=True)
    model = RobertaModel.from_pretrained(modelname, local_files_only=True)
...
def get_embedding_for_single_text(text: str):
    if not isinstance(text, str) or not text:
        print("Warning: Input to get_embedding_for_single_text is not a valid string.")
        return np.zeros(model.config.hidden_size)  
    inputs = tokenizer(text, padding=True, truncation=True, return_tensors="pt", max_length=510)
    inputs = inputs.to(device)
